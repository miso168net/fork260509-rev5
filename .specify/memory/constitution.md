...
- **F3 fail-* 方向**：全鏈 **fail-open**；fail-closed **恰兩處**——①**寫端自鎖拒寫**（會把操作者自己鎖在門外的規則 MUST 拒寫、零落庫、零重載）②**轉發鏈跳數逾上界即拒絕**（見 F7）。每次降級 MUST 發**結構化告警**。★本條的 fail-closed 例外集由一項擴為兩項時，曾辨過「這算不算方向性反轉」（ADR 0043）：結論是**不算**——降級鏈的方向一條未變，新增的是原本不在降級矩陣裡的判定腿。**若日後要把任一**降級**腿由 fail-open 翻成 fail-closed，那才是 §V.3 的方向性反轉＝MAJOR。**
...
- **F7 轉發鏈跳數上界**（004 追加）：鏈跳數逾上界者 MUST **拒絕該請求**（不服務、不進密碼雜湊驗證），且 MUST **仍落一列稽核**——拒絕不得換來證據缺口。該列 MUST 被**帳號維計數排除**、被**來源維計數納入**：排除保護受害者（否則攻擊者拿受害者帳號名發敵意鏈即可無限期鎖死該帳號、連密碼都不用試），納入封頂稽核表成長（來源維鍵為攻擊者自身位址）。★**與島 E 的「拒絕零稽核列」不矛盾**：島 E 那句的目的（其括號逐字）是「拒絕不得消耗受害者的額度」，零稽核列是當時達成它的**手段**（因為計數查詢數所有列）；本條以「寫列＋帳號維計數排除」達成**同一目的**，兩者是同一不變式的兩種實作。
- **F8 稽核轉錄的複驗性**（004 追加）：稽核列的轉發鏈欄 MUST **保留判定窗**，且 MUST 與判定軌**共用同一份取窗實作**——使「稽核欄 ⊇ 判定窗」為結構性保證，真實來源必定可由該欄複驗。★兩份切分規則必然漂移。
