...
- **B append-only 日誌**（例：三 log 表）：只 `created_at` NN（＋operator 類 domain 欄）；**無 soft-delete、無 update、不可竄改**；MUST NOT 加 `updated_*`/`deleted_*`（retention 水平線刪除不屬「竄改」——權威釋義＝§I.7 島 J3、本句為交叉指針）
...
**I. 使用者域治理**（007-user-password-admin 進場；I1～I5 沿 rev4 已驗證形〔rev4:ADR 0053 總綱／rev4:ADR 0054 密碼政策〕、**I7 no-escalation 包含規則為本刀新拍板**〔ADR 0063〕；字母沿 rev4 記 I、★**I6 位刻意空缺**＝rev4 之密碼經手與首登強制換密〔rev4:ADR 0067〕本刀不做、留 B-134，日後兌現時回填該位）
- **I1 寫端鎖序與 lock-then-redecide**：一切以**既有**使用者為標的之使用者域寫端 MUST 於交易起手取得與登入／換發同源的 per-user advisory 鎖；域內固定鎖序 MUST 為 advisory(uid)→標的使用者列 `FOR UPDATE`（復原用已刪列版）→角色列（僅比對需要時讀、識別升序）→角色指派列寫入，禁反向；一切守門判定 MUST 鎖內重驗（lock-then-redecide、永不信 pre-read）。新增使用者豁免 per-user 鎖（新識別對並發不可見；並發同名保護＝帳號名活性唯一約束）；批次刪除依識別去重升序逐一取鎖。帳號名與信箱之活性唯一索引為復原衝突守門之顯式前提。★advisory key space 沿用登入之 uid 鍵（同用途擴消費者、與島 H1 域鎖之高位自描述常數不碰撞、核過）。★方向反轉（拆散序列化、改回無鎖 pre-read）＝MAJOR。
- **I2 斷權即時性與分碼不互換**：停用／刪除／重設密碼 MUST 同交易撤銷標的全部 active 票（rotated 列不動、重放仍走既有重用偵測）並落 session 事件；管理員踢除同形、但為**獨立 reason**。動作序 MUST 權威優先（業務寫＋票作廢＋稽核同一交易落定→commit→失效廣播 best-effort、存活時間覆蓋換發憑證壽命）；廣播失敗只結構化告警、**權威儲存為準**。換發流程 MUST 於鎖內重驗使用者活性（啟用且未軟刪），不活即拒——此為既有 token 狀態機**新增判定腿**、非 fail 方向反轉。MUST NOT 為此新增每請求活性判定。撤銷類、單一會話頂替類與管理員踢除類之體驗碼與文案鍵 MUST NOT 互換（三者各自對應、勿「統一」）。★方向反轉（拔鎖內重驗、改廣播優先、改為每請求活性判定）＝MAJOR。
- **I3 seed 帳號與自身結構保護**：前三個種子帳號 MUST 不可刪；第一個（Super）MUST 恆禁停用、恆禁解除其超管角色指派（不因操作者身分而異）——系統恆有至少一個活躍且啟用的超級管理員（結構保證、不需動態計數）；Super MAY 被踢除、被重設密碼。操作者 MUST NOT 刪除／停用／踢除自己、MUST NOT 變更自己的角色指派、MUST NOT 以管理端重設自己的密碼（自助改密走自助端點）；操作者 MAY 改自己的非角色欄與會話政策。守門為碼內常數形。
- **I4 軟刪硬刪指派＋復原零回灌**：使用者軟刪 MUST 同交易硬刪其全部角色指派列（零幽靈掛載、角色域掛載計數守門保持誠實）；復原 MUST NOT 回灌任何指派（復原後零角色、須重新指派）、狀態保留刪除前原值；同帳號名重建之新使用者 MUST NOT 經任何路徑繼承舊實例角色。批次刪除逐項驗證、任一違規（含已刪識別）**整批拒**（no-partial、單一交易）。
- **I5 密碼政策單一驗證點＋登入不驗＋三重不洩**：密碼政策驗證 MUST 為單一驗證點（建帳／管理端重設／自助改密三入口共用、零分叉）、政策鍵單一快照讀取、違規 MUST 一次收集全部；長度單位＝字元、另加固定位元組上界 ≤登入端形制上限；「禁止密碼與帳號名相同」＝大小寫不敏感相等。★**登入路徑 MUST NOT 驗政策**——政策是設密面的守門、不是登入面的守門；把它掛上登入即令既存帳號的合法憑證因政策事後調整而失效。密碼明文與雜湊 MUST NOT 洩漏於任何面：承載密碼之 DTO 除錯輸出 MUST 遮蔽（不得預設印出）；操作稽核 payload MUST NOT 含密碼明文／雜湊；API 回應 MUST NOT 含密碼；密碼雜湊之**生成**（hash）MUST NOT 於持有列鎖期間計算（三入口一律於取鎖前算好再進鎖）；雜湊**驗證**（verify）作為守門判定，依島 I1 於鎖內重驗執行（ADR 0068）。★方向反轉（拆單一驗證點、拔遮蔽、把政策掛上登入路徑）＝MAJOR。
- **I7 no-escalation 包含規則**（rev5 專屬新條）：使用者域寫端授權 MAY 由超管於運行期下放給其他角色（seed 預設仍 super-only、下放為可開關能力、非預設態）；下放後**一切受規則約束之寫端**（使用者域寫端全集＋帳號維解鎖）MUST 於鎖內、任何寫入前判 `T ⊆ A ∧ N ⊆ A`——A＝操作者現役角色集（濾軟刪與停用角色、DB-fresh；★持 `R_SUPER` 者之 A 視為全集）、T＝標的全部指派列（**不**濾角色狀態）、N＝寫後標的角色集；違者 MUST fail-closed 拒絕（純 key、不洩漏角色差集、零變更零稽核）。seed 保護（I3）與 self 諸不 MUST **先於**本規則判定；同級互管允許（零特例）；來源維解鎖不套。判定 MUST 以具名純函式單點實作、諸寫端共用；前端 MUST NOT 預判本規則（後端為唯一裁判）。★方向反轉（拔守門、改 fail-open、改由前端預判、把某支受約束寫端移出射程）＝MAJOR。
- 常數（seed 帳號識別集、政策鍵名與門檻、設密冷卻與改密節流之窗與次數、事件 reason 字面集、自助路由白名單成員、advisory key 值）＝活書／ADR 級、不入條文。

**J. 稽核域 retention 與 reporting**（008-audit-settings-pages 進場；J1／J3／J4／J5 沿 rev4 已驗證形〔rev4 憲法 §I.7 島 J 段；設計理據 rev4:ADR 0057 四源讀端／rev4:ADR 0058 purge 執行面／rev4:ADR 0059 PII 打碼〕、**J3 之零部分成功機器證句為本刀新拍板**〔ADR 0077〕；字母沿 rev4 記 J、★**J2 位刻意空缺**＝rev4 之 access-log 寫入端 fail-open〔rev4:ADR 0060〕本刀不做、留 B-016，日後兌現時回填該位；★**J5 位隨本刀填入**＝rev4 之解鎖稽核先於生效，rev5 對應行為已 as-built 且有源序機器守〔004-ip-trust-anchor U-K／T052〕）
- **J1 讀端 read-only＋授權面 seed 預設僅超管**：稽核查詢面（操作日誌／存取日誌／登入嘗試／會話事件四源）MUST 純唯讀（handler 零業務寫入、零狀態變更）；授權面 **seed 預設僅 `R_SUPER`**，MAY 由超管於運行期下放給其他角色（下放為可開關能力、非預設態——形照島 I7）；回應 MUST 逐欄構造、不序列化原始列；資料源集合增減＝新 ADR。★方向反轉（讀端開寫入面、把 seed 預設改成非 super-only）＝MAJOR。
- **J3 purge 唯一形狀＝時間水平線＋原子自記**：稽核資料唯一刪除路徑＝來源表白名單×保留天數下限守門之水平線整段刪除——構造上禁挑列／任意條件刪；每次 purge MUST 同一交易自落操作日誌自記（含來源表、天數、刪除筆數；刪 0 列照落）；操作日誌源之水平線 DELETE MUST 固定豁免歷史清理自記列（後設證據永久保留）；自記寫入失敗 MUST 整筆回滾（零刪除、零自記、回錯誤）——「回報刪 N 筆、實際零刪除」MUST 構造不可達並以 fault-injection 級測試釘住。fail-* 方向：purge 全鏈 **fail-closed**（任一步失敗＝整筆回滾）。水平線 retention 刪除不屬 §I.6 變體 B 所禁之「竄改」——**本條為其權威釋義**。★方向反轉（開挑列刪、拆自記／豁免、容忍部分成功）＝MAJOR。
- **J4 PII 顯示打碼單點**：稽核讀端 payload 之個資遮蔽 MUST 於後端序列化單點完成（前端 MUST NOT 經手原值）；遮蔽鍵清單＝常數（擴充＝改常數＋補測試）；落庫面白名單（密碼明文／雜湊永不入列、島 I5）為政策本體、不回溯清洗。★方向反轉（拆單點、改前端打碼）＝MAJOR。
- **J5 解鎖稽核先於生效**：手動解鎖動作序 MUST 為操作稽核於權威儲存落定成功後才執行生效步；稽核寫入失敗 MUST 中止生效——「已生效但零稽核列」MUST 構造不可達（生效步失敗後重試多記嘗試列＝明文接受）。★方向反轉（生效先於稽核、容忍生效後稽核失敗）＝MAJOR。
- 常數（`PURGE_MIN_DAYS` 值、來源表白名單成員字面、遮蔽鍵清單、分頁 clamp 界與固定排序、拒因鍵字面）＝活書／ADR 級、不入條文。
...
| **★BASE-WEB-LOGIN-CAPTCHA-WIRING** | (i) 登入頁 captcha 軟區 | `src/store/modules/auth/index.ts`（修改型）／`src/views/_builtin/login/modules/pwd-login.vue`（修改型＋新增型） | login 簽名加 captcha 參並串通失敗 msg 回傳鏈；軟區為條件渲染、**非軟區時零行為變更** |
...
**Version**: 1.10.0 | **Ratified**: 2026-08-04 | **Last Amended**: 2026-09-01
...
- 1.9.1（2026-08-29）：**§I.7 島 I5 末句澄清——受「持有列鎖期間不得計算」約束者為雜湊之生成（hash），驗證（verify）作為守門判定依島 I1 於鎖內重驗執行**。動機＝007-user-password-admin 之 U4 落地自助改密時，島 I5 末句與島 I1 在 `change_own_password` 五步序第④格（argon2 verify）上正面相碰：verify 既是雜湊運算（I5 禁）又是守門判定（I1 命），且其比對基準是鎖內那一列的 PHC、外移即回到 research R2 已明文拍板消掉的 rev4「鎖外預讀＋鎖內 PHC 重比」形。判定＝**不是真衝突，是承襲 rev4 條文時字面不夠精確**（末句所在段落前三句全在講不洩漏，第四句講的其實是鎖窗管理，而該關切只對生成成立）。ADR 0068；PATCH（§V.3「釐清」——不新增禁令、不放寬既有禁令射程、不改變任何 as-built 行為；島 I5 的 ★方向反轉條款不受影響，**碼零改動**）。user 親決 2026-08-29（三選項：改條文／立例外 ADR／verify 外移回 rev4 形，取改條文）。
- 1.9.0（2026-08-27）：**§I.7 第九座行為島入憲（島 I 使用者域治理、含 I7 no-escalation 包含規則）＋§III.2 `BASE-WEB-MANAGE-PAGE-WIRING` 加用途 (v)(vi)（user 管理頁接真＋個人中心改密頁）＋`BASE-WEB-LOGIN-CAPTCHA-WIRING` 明文凍結位 (ii) 開立（登入表單規則放寬）**（007-user-password-admin 之 U0）——I1～I5 沿 rev4 已驗證形（rev4 憲法 §I.7 島 I 段原文；設計理據 rev4:ADR 0053 總綱／rev4:ADR 0054 密碼政策），落字差異六處：I1 改寫為方向面＋advisory key space 不碰撞句（島 H1 已凍結該紀律、此處只擴消費者）／I2 補「換發鎖內重驗＝新增判定腿、非 fail 方向反轉」與三 reason 不互換（rev4 只寫兩類、rev5 為三類）、刪即時性量測句（活書級）／I3 之 self 四不擴為諸不（新增「MUST NOT 以管理端重設自己密碼」）／I4 刪 fail-fast 措辭（與 no-partial 同義、避免兩詞並存）／★I5 **新增「登入路徑 MUST NOT 驗政策」整句**（rev4 未寫；rev5 seed 三帳號密碼 6 字元＜政策 min 8，不寫即結構性自鎖、本刀有機器守）／I7 全新。★**I7 no-escalation 包含規則＝rev5 專屬新條**：寫端授權 MAY 運行期下放，下放後 MUST 判 `T ⊆ A ∧ N ⊆ A`（A＝操作者現役角色集、**持 `R_SUPER` 者之 A 視為全集**逐字入條文〔U0 題① user 親決取具名；先例＝同節 G6 條文即逐字寫「非 R_SUPER 角色」〕；T＝標的全部指派列、不濾角色狀態；N＝寫後角色集），seed 保護與 self 諸不先判、同級互管允許、來源維解鎖不套、前端 MUST NOT 預判；反轉＝MAJOR。★**I6 位刻意空缺**＝rev4 之密碼經手與首登強制換密（rev4:ADR 0067）本刀不做、留 B-134（custody 表只借時戳），header 括號採完整形載明以免跳號只能靠猜（U0 題② user 親決）。用途 (v) 修改型檔級名單**定數恰 2 支**（`user/index.vue`＋`modules/user-operate-drawer.vue`；★同目錄 `user-search.vue` 經 rev4 與最原始源 `example` 基線**兩向逐位 diff 零差異**判定零改動、明文不入名單，出現任何 diff＝紅）、(vi) **恰 1 支**（`user-center/index.vue`）、LOGIN-CAPTCHA (ii) **恰 1 支**（`pwd-login.vue`）；三用途之 base-web 既有 i18n 檔（兩語 locale 之 `page:` 樹＋`app.d.ts` 之 page 型節）逐支寫出，`backend:` 樹新鍵授權仍在**既有** I18N-WIRING (ii)(iii) 射程內、不隨本次擴列。同批刪 LOGIN-CAPTCHA (i) 列紀律欄之「用途 (ii)（`formRules` 放寬）不在授權內」句（凍結位開立之必然後果、不刪即自相矛盾）。no-escalation 兩實作位射程分工（middleware 四參掛點恆放行＝路徑級上限位；handler 鎖內具名純函式＝body 級指派集——中介層取不到 body 亦取不到鎖內的 T）與按鈕 gating 例外釋義（判準＝該頁 menu 維政策是否僅 `R_SUPER`；實測 `manage_user`＝{R_SUPER, R_ADMIN} 故七碼逐鈕 gating、`manage_role`／`manage_menu` 僅 R_SUPER 故門在頁級）由 ADR 0063 款三承載、**ADR 級不入條文**（補充 ADR 0022 決定 3、不 supersede；ADR 0053 款二 (iii) 列之 role 頁不 gating 拍板不變）。ADR 0063；MINOR（§V.3 之「行為島隨刀進場」＋「軌道授權邊界擴展」兩款；非 MAJOR——零既有入憲不變式反轉）。兩題 user 親決 2026-08-27。
