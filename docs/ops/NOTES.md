...
- **★下一動作＝008-audit-settings-pages 之 `/speckit-specify` 手動起手**：下一刀本體已拍
  （2026-08-31）＝B-008 餘兩張 view＋audit 五端點；brainstorm 定稿
  `docs/brainstorms/008-audit-settings-pages.md`（兩題親決＋設計核可、拍板②承載＝ADR 0076）。
  ★specify **手動**起手（feature branch pre-hook 才會跑、spec 才不落 default branch）。
