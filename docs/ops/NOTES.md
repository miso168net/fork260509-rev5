# NOTES — 當前意圖／下一步

- **創世序列（B0～B11）全部收官**：B8b 移植驗收後段與 B11 的 K1／K2 處置流水（ADR 0009）
  已收單，創世 DoD 全數關帳；啟動書 §5 自此為候選清單史料（K1 查用點＝各刀階段 0、
  K2 查用點＝BACKLOG 條目本文）。
- 帶 migration 的刀沿用 001 立的紀律：收刀前必跑 refresh＋演進帳登記＋三閘綠（RUNBOOK §10）。
- **提前批進行中**（user 拍板：B12 前先清 bash→python 轉換帳、ADR 0010/0011）：B-035 已
  收單（merge 2d22fb6）。下一步＝**B-036**（decrypt「passphrase 只打一次」＋安全姿態 ADR、
  等價驗收需 user 在場跑真密文）→ B-037（generate-secrets＋setup-reaper 尾批、刪 preflight
  舊 .sh）；B-038（age 容器化）與 B-039（pre-push 測試矩陣）獨立、可插隊。
- 提前批清完＝**B12 第一把功能刀**（後端首刀、spec-kit 流程原樣）。brainstorm 直接輸入＝
  B-014（entity Relation 補強）＋B-001（承襲盤點閘評估）＋K1 對應域條目；★開刀前宜先決
  B-031（prod 是否入 roadmap）與 B-032（團隊組成前提），兩題影響刀序與範圍。
- 維護批候選堆積：B-002/B-005/B-007/B-010/B-013/B-015（首個維護批）；
  B-006（首筆真登記前）；B-011（postgres 升版前）。
