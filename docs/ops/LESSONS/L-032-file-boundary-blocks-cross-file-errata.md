---
promoted_to: CLAUDE.md §2 防呆⑥（清單外檔案需要動→blocked 升級主線＝防法①之邊界動作）＋§4 時態分離（活書恆現在式、過去式住 git＋events＝防法③「史述保留、現在式改對」）；防法①前半（agent prompt 通則：改變數字／集合大小／方向／名稱／單一權威 MUST `grep -rn` 全 repo 枚舉並逐處回報）CLAUDE.md 零承載、候選位＝§2 編排範本之不可違反項；防法②之枚舉面**半載**——§4 勘誤條之 `docs-sync.py errata` 掃描面止於外層 tracked 檔（`git ls-files`、子庫僅 gitlink）、不含子庫 `src/**`（與 L-038 同判；本條實暴④之八處有六處在 rust-api 子庫、errata 掃不到）、時點半（跨檔假述枚舉列入單元收尾①固定動作）未載，候選位＝errata 加子庫掃描腿（docs-sync）＋§2 單元收尾①；皆不在本單元允許面、待下一把動各該處的刀（2026-08-30 B-091 盤點）
---
