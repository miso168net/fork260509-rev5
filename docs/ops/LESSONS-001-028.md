# LESSONS-001-028 — 已收刀教訓（L-001 ~ L-028）

本卷＝主檔 `LESSONS.md` 滿卷分出（Lint07 單卷上限 25000 tokens）。條目格式與紀律同主檔；
**本卷無 next-id**——配號永遠只在主檔（`_parse_next` 只讀 `docs/ops/LESSONS.md`，
把 next-id 搬離主檔會讓整族 ID 配號停擺）。ID 唯一性由 `lessons_paths()` **跨卷**檢查。
★分卷以**刀**為界：本卷收 004-ip-trust-anchor **之前**各刀的教訓，主檔留當前刀。
★引用一律用 **ID**（`L-NNN`）不用行號、亦不用檔名——條目住哪一卷不影響任何既有引用。
