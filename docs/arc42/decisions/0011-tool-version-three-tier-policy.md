...
操作經手、tag 可被重推而 digest 不可變是 rev4:P1.1 立論本身。故收斂為三分類。
...
| ② 機密管線常駐件 | 每次機密操作或每次 commit 經手、版本＝行為＝閘的跨機一致性 | 釘版＋機器斷言（現狀不動） | sops 容器（digest、rev4:P1.1）、betterleaks（bootstrap 斷言） |
