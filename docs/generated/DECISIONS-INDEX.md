...
| 0064 | accepted | 2026-08-30 | BizData 明細通道射程嚴限密碼二鍵——澄清 ADR 0022 §2② 之事實誤述，並以「操作者自陳輸入的評估結果」為判準結掉 B-024③ 受眾邊界重評 | — | — | — |
| 0065 | accepted | 2026-08-30 | 自助路由白名單為後端碼內常數、恆併於 Casbin 過濾之後——rev5 缺 rev4 的頁級豁免條文，改以「業務 menu 射程＋hide_in_menu seed 實值」二支柱立論 | — | — | — |
| 0066 | accepted | 2026-08-30 | 設密冷卻與改密舊密節流的定形——兩個防重試機制刻意分家；門檻進不進設定表的判準＝「正確值是否隨組織而異」；改密節流 fail-open 且自成觀測源 | — | — | — |
| 0067 | accepted | 2026-08-30 | ADR 0042 第 2 項之解鎖入口歸屬預測訂正（實落使用者管理頁、可見落差同批解除）＋ADR 0053 款四判定面同步觸發矩陣擴入 user 域三支寫端（含訂正後完整矩陣） | — | — | — |
| 0068 | accepted | 2026-08-29 | 憲法 Amendment 1.9.1——§I.7 島 I5 末句澄清：受「持有列鎖期間不得計算」約束者為雜湊之**生成**（hash），**驗證**（verify）作為守門判定依島 I1 於鎖內執行 | — | — | — |
| 0069 | accepted | 2026-08-30 | 手機欄「≤32」判為欄位描述性上界、非 MUST 守門——四個權威取「無守門」的那三個，不為一個沒有來源的數字憑空造使用者可見拒因 | — | — | — |
