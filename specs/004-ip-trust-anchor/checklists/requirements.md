...
- **零 [NEEDS CLARIFICATION]**：brainstorm 開場四題已由 user 逐題拍板（Clarifications 首節
  ...
  製造雙權威）。兩處寫成「plan 期定案」者**不是** unresolved marker，而是帶可執行前置的
  requirement——FR-009（轉發鏈畸形段之處置：**MUST 明確定義並以測試釘住**）與 FR-041
  （路由生成檔與 fork-delta 檢查工具的相容形：**plan 期實測工具行為後定案**），兩者皆已
  凍結「必須做什麼」、待定的只有「照哪個既有語意」。
