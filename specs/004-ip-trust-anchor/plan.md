...
2. **§III.2 base-web inline**：**涉及——授權以 Amendment 先行取得**。本刀動 base-web 既有檔
   ...
   加 `page.manage.ipRule.*` 必須手動補型；既有 (iii) 只授權 `backend` 型節、涵蓋不到）＋
   **路由外掛重算產出的四支生成檔**（`src/router/elegant/{imports,routes,transform}.ts` ＋
   ...
   授權鏈＝ADR draft → user 親決 →
   accepted ＋ §III.2 新增一條軌道 ＋ §I.7 島 F 條文 ＋ bump 1.3.1→1.4.0 ＋
   `docs-sync generate`。★**硬序約束**：Amendment accepted **之前不得動任何 base-web 既有檔**
   （research R10 之 **U0→U5／U7／U8** 硬序——U5／U7 亦動 base-web 既有檔，其 msg 鍵須落
   兩語 locale 與 `app.d.ts` 型節）。★第三塊採**產物檔紀律**（禁手改＋重算冪等驗收、明文不要求
