...
# ★★本檔的登入走查**一律走 curl，不可逕行改用 UI 重現**（L-047；2026-08-17 CDP 走查實暴）：
#   前端登入表單另有一道**後端沒有的**格式閘 `REG_PWD = /^\w{6,18}$/`（base-web/src/constants/
#   reg.ts、upstream 基線碼）⇒ 本檔用的錯密碼字面 `"wrong"`（5 字元）與 `"x"`（1 字元）
#   **在 UI 上送不出去**。★危險的不是送不出去，是**失敗形**：`NForm.validate()` 直接 reject、
#   **零 API 請求、零 toast**，畫面唯一線索是欄位下方一行小字 feedback ⇒ 與「後端掛了」
#   「請求被中間層吞掉」完全同形，排查方向會整個走偏。
#   要在 UI 上重現本檔任一節，密碼字面須換成合規值（例：`"wrongpw"`／`"badpass1"`）——
#   語意不變（都是錯密碼），但兩條路徑自此可互相對照。
