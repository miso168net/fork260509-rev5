...
★`real_ip` 在該態下**仍有值**（取自判定窗）——來源維計數要數這些列（FR-050），沒有位址
就無從歸戶。
...
| F3 | 全鏈 fail-open；fail-closed **恰兩處**＝①寫端自鎖拒寫 ②轉發鏈跳數逾上界即拒絕（F7）；每次降級必發結構化告警 |
...
| **F7（本刀新增）** | **鏈跳數逾上界 MUST 拒絕該請求**（不進密碼雜湊驗證）且 MUST 仍落一列稽核；該列**帳號維計數排除、來源維計數納入** |
