...
① 它標記「**這條鏈逾上界、未被推理採信**」，語意上不與其餘七態同軸——其餘七態答「這個
   位址多可信」，它答「這條鏈根本沒被推理」。讀稽核列時 MUST 先分流本值再談信心。
   ★**MUST NOT 讀作「該請求未被服務」**（U-M 規格審查修正）：標記由 `request_context_mw`
   **全域**施加、拒絕只在**登入端點**（FR-047 射程）⇒ `sys_operation_log` 上出現此值代表
   「鏈逾上界但**請求仍被服務**」；`sys_login_attempt` 上則因該表唯一寫入者是 login 而恆為
   「被拒」。**兩表同一字面、語意不同**，查詢與報表 MUST 分表判讀。
   ★帶此值的列，其原本的錨結論**不另存**（一欄一值；另立欄即破「恰四欄」封條、須另立 ADR）。
   ★`real_ip` 在該態下**恆有值**——來源維計數要數這些列（FR-050），沒有位址就無從歸戶。
   **但出處視判定腿而定**：層②③取自判定窗，層①直連腿／④回退腿／通道覆蓋層取自傳輸層對端
   或訪客標頭（本標記由 `apply_chain_overflow` **無條件**套在三層矩陣**之後**，故各腿皆可
...
| `x_forwarded_for` | ★**判定窗**之淨化轉錄（最右 `MAX_XFF_TOKENS` 欄，再以字元上限兜底；零 CR/LF）——FR-046／憲法 F8 改此形，**原「最左 1024 字元」已廢**（方向與判定軌相反＝證據代筆面） |
| `ip_confidence` | 八態字面（此前＝單一字面）★本表可出現 `chain_rejected`＝「鏈逾上界但**請求仍被服務**」（標記全域、拒絕限登入；語意詳 §1.2①），**不是**「被拒」 |
| `peer_ip`（新增消費） | 傳輸層對端 |
...
★**七 vs 八的分野**：**三層判定矩陣**（peer 閘→Tier-1 CDN 錨→Tier-2 最右非受信，見
research R4）只產出**前七態**；第八態 `chain_rejected` 是**矩陣之前的短路**——鏈跳數逾
`MAX_XFF_TOKENS` 時直接定案、不進矩陣。故「信任錨七態」（＝矩陣的七個出口）與
「`ip_confidence` 八態」（＝欄的值域）兩種說法**都對**，講的是不同的東西；引用時看清楚
主詞是「矩陣的出口」還是「欄的值域」。
★**溢出旗標刻意不另立欄**：`RequestContext` 為**恰四欄**封存型（spec FR-011＋
`compile_fail,E0451` doctest），加第五欄即破封條、須另立 ADR。改以「還原結論的信心即
`ChainRejected`」承載 ⇒ 零新欄、封條不動，且值仍出自**同一個** `Confidence::as_str`
出口 ⇒「全 repo 無任何路徑能寫出值域外的 `ip_confidence` 字面」自動成立。
★`real_ip` 在該態下**仍有值**——來源維計數要數這些列（FR-050），沒有位址就無從歸戶。
**但出處視判定腿而定**（★勿寫成「取自判定窗」：本標記由 `apply_chain_overflow` **無條件**套在
三層矩陣**之後**，各腿皆可落此值）：層②③取自判定窗，層①直連腿／④回退腿／通道覆蓋層取自
傳輸層對端或訪客標頭。逐字同 §1.2① 該格——**兩處不得分岔**。

`cdn_verified`／`proxy_clean`／`direct`／`cdn_anchored`／`proxy_soft`／`cdn_mismatch`／
`fallback`／**`chain_rejected`**。判定矩陣＝research R4（前七態）；**dev 經反向代理可達二態**
＝research R7；第八態的可達條件＝鏈跳數 > `MAX_XFF_TOKENS`（矩陣之前短路）。
...
| F3 | 全鏈 fail-open；fail-closed **恰兩處**＝①寫端自鎖拒寫（含判定所需之操作者來源位址不可得時同向） ②轉發鏈跳數逾上界即拒絕（F7，**限登入端點**）；每次降級必發結構化告警 |
...
| **F7（本刀新增；射程於憲法 v1.6.2 收窄）** | 鏈跳數逾上界者**於登入端點** MUST **拒絕該請求**（不進密碼雜湊驗證）、**其餘端點 MUST 標記但照常服務**（標記全域、拒絕僅登入——逾限只有中介層算得出來，下游拿到的已是判定窗、不可重算）；逾限請求 MUST 仍落一列稽核，該列**帳號維計數排除、來源維計數納入** |
| **F8（本刀新增）** | 稽核列的轉發鏈欄 MUST **保留判定窗**且與判定軌**共用同一份取窗實作**——★結構性的是「同一份切分、同一組欄」這一半；**複驗性帶兩個成立條件**（(a) `real_ip` 由鏈推導 (b) 窗未逾字元上限），**不得寫成無條件保證**（憲法 v1.5.1 收窄；原 v1.5.0 措辭經逐位元反例證偽）|
...
| 請求上下文缺席 | fail-open（**閘門與節流面**） | 閘門放行；來源維整層跳過 | 告警 |
| 請求上下文缺席（**IP 規則寫端**） | **fail-closed** | 拒寫、回 `5000`——★非第三條 fail-closed 腿，而是憲法 F3① 自身的降級方向（v1.6.0 補句）：四寫端 MUST 同交易落稽核列，而 `real_ip` 為 `inet NOT NULL` ⇒ 無來源位址則業務列＋稽核列整體寫不出來；補佔位位址＝證據代筆 | 業務錯誤回應 |
