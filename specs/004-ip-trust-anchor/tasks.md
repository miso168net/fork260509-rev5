...
- ★**Lint24 同步律（跨子庫；閘讀「工作樹」、不讀 git index）**：`error.rs`／handler 新增一個
  後端實發 msg key ⇔ `base-web/src/locales/langs/zh-tw.ts` 補同名鍵，兩邊須在**同一次工作樹
  編輯內**齊備。★孤兒鍵窗**不得跨越任何一次外層 commit**：兩子庫先各自 commit（次序自由），
  再以**一顆**外層 commit `git add rust-api base-web` 同時 bump 兩顆 pin。★外層 commit 一律
  不得 `--no-verify`。
...
- [ ] T001 ★主線任務（user 親決）：撰寫憲法 Amendment 的 ADR draft 於
  ...
  ＝兩語 locale 之 `route:`／`page:` 樹、型別宣告檔對應節、★**路由外掛產物三檔**；★產物檔紀律
...
- [ ] T002 ★主線任務（user 親決後）：ADR 轉 accepted＋更新 `.specify/memory/constitution.md`
  ...
  **DoD：lint 全綠；此 commit 落地即解除 base-web 既有檔硬閘**
...
- [ ] T004 三個新依賴釘版（research R3 三源核對表）：`rust-api/Cargo.toml` 的
  ...
  （★三支皆取 default features——pub/sub 在既有 `connection-manager`＋`tokio-comp` 下即可用、
  **零新 feature flag**）。**DoD：容器內 `cargo build` 綠；`Cargo.lock` 成長與 futures-util
  升版逐筆記入 commit message**
...
- [ ] T014 `rust-api/server/tests/common/mod.rs` 的 `stub_state` 同步七欄
  ＋`rust-api/server/src/router.rs` 內的第二份 stub_state 亦同步
  ＋`rust-api/server/src/handler/system_settings.rs` 之 `real_app()` 建構點同步。
...
- [ ] T030 [US2] `rust-api/server/src/obs.rs`：本刀四類降級各 pre-register 一支計數器
  （信任模型載入失敗／規則載入失敗／門鈴故障／IP 閘阻擋），★**啟動即註冊 0**（不因「事件
  尚未發生」而整條訊號缺席）。**DoD：計數器 render 文本比對測先紅後綠**
...
- [ ] T038 [US3] 後端 msg key 五鍵落地（`biz.ipRule.{invalidRuleType,invalidCidr,conflict,
  notFound,selfLock}`）＋★**依 Lint24 同步律**在**同一次工作樹編輯內**補
  `base-web/src/locales/langs/zh-tw.ts` 五鍵（譯文語意見 `contracts/msg-keys.md`）。
  **DoD：`python3 tools/docs-sync.py lint` 綠**
...
- [ ] T041 [US3] ★**新軌道射程內的既有檔改動**（★T002 未 accepted 前不得動）：
  ...
  `manage.ipRule.*`（★兩語鍵集 MUST 相等）＋`base-web/src/typings/app.d.ts` 對應型節（如需）
  ＋`base-web/src/router/elegant/{imports,routes,transform}.ts` **由外掛重算產出**
  （★**禁手改**）。每處依 fork-delta 紀律標記（★產物三檔**除外**——依 T002 條文不要求逐行標記）。
  **DoD：`pnpm typecheck` 綠；`fork-delta-lint` 綠（含新軌道名冊斷言）**
- [ ] T042 [US3] 本刀兩支新機器守（★各附 self-test、植入反例必紅——ADR 0024 非 vacuous）：
  ①`views/manage/**` 零 `v-html`／`innerHTML` 斷言（掛 pre-commit）②路由產物三檔**重算冪等
  檢查**（重跑外掛後與版控內容零差異）。落點沿既有 lint 家族。
  **DoD：兩支綠；★兩支的 self-test 各自證明「拿掉守門即紅」**
- [ ] T043 [US3] 走查 quickstart §3＋§6：CDP 三方對照（22080 vs 42080），六步全程＋
  主機位元正規化＋衝突提示＋備註欄標記字元顯示為字面；另驗防自鎖拒寫且**零寫入**。
  **DoD：六步全通；`SELECT count(*)` 證明自鎖案零寫入**
...
- [ ] T045 [P] [US4] `rust-api/server/src/throttle/mod.rs` 之 `#[cfg(test)]`：計數桶粒度
  ...
  ★L0 短路兩案（顯式 allow 規則→跳節流；**結構豁免段→不跳**）。**先確認紅**
...
- [ ] T047 [US4] `rust-api/server/src/throttle/mod.rs` 擴來源維：`DIM_IP` 常數＋`ip_bucket`
  粒度導出＋`parse_unlock_marker`（★格式契約＝**unix 秒十進位字串**；不可解析→視為無標記）
  ...
  否則放行）＋解鎖標記讀取故障 **fail-closed**。
  ★同批修 **L-026**：三處上下界共用同一顆具名餘裕常數、註解與失敗訊息對齊碼。
  **DoD：T045 由紅轉綠；★既有帳號維 321 支測試零回歸**
...
- [ ] T054 [US5] 後端 msg key 一鍵（`biz.throttle.invalidUnlockTarget`）＋★依 Lint24 同步律
  同批補 `zh-tw.ts`；`rust-api/server/src/router.rs` 加**第六條 ROUTES**
...
- [ ] T060 [P] ★**wire 裁判面重評落帳**：`additionalProperties` 維持已知態不動（動快照生成器
  會牽動既有全部路由的裁判行為、屬 wire 地基面），僅更新該面的警示句
...
- US3：T031／T032 可分派；T033／T039 可分派；★T040 與 T041 **不可**併行（T041 的產物三檔由
...
| U-E | T013~T015 | `state.rs`、`main.rs`、`tests/common/mod.rs`、★`router.rs`（僅其 `mod tests` 的第二份 stub_state）、★`handler/system_settings.rs`（僅其 `real_app()` 建構點）、兩支 lint |
...
| U-H | T031~T038 | `contract.rs`、`router.rs`、`model/{audit,mod}.rs`、`facade/{sys_ip_rule,sys_operation_log,mod}.rs`、★`handler/{mod,ip_rule}.rs`、`tools/schema-gate.py`、`zh-tw.ts` |
| U-I | T039~T043 | `rev5-ip-rule.{ts,d.ts}`、`views/manage/ip-rule/` 三檔、兩語 locale、`app.d.ts`、`router/elegant/` 三檔、`tools/`（兩支新守） |
| U-J | T044~T049 | `facade/sys_login_attempt.rs`、`throttle/mod.rs`、`handler/auth/login.rs` |
| U-K | T050~T055 | `contract.rs`、`router.rs`、★`handler/{mod,throttle}.rs`、`throttle/mod.rs`、`cache/mod.rs`、`zh-tw.ts` |
| U-L | T056~T063 | `RUNBOOK.md`、`ARCHITECTURE.md`、`BACKLOG.md`、`LESSONS.md` |

★清單以「★」標出者為**模組註冊檔與散布點**（`lib.rs`／`handler/mod.rs`／`facade/mod.rs`／
`router.rs` 的第二份 stub）——漏列即 fix agent 撞清單外檔而 blocked（防呆⑥「清單只縮不擴」）。
...
★**U-I 對 U-A 是硬序**（憲法授權在前、動 base-web 既有檔在後）；U-C~U-H、U-J~U-K 為純後端、
不受該閘阻擋。
