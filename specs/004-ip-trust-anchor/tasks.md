...
- [x] T058 [P] `docs/ops/BACKLOG.md` 帳面處置（spec §2.2）：**B-019 關帳**／~~B-073 關帳~~（★**已於 U-K 收尾提前關帳**——其斷言「unlock marker 恆 NULL、無寫入者」自 T052 起為假、屬現在式假述，不宜留到 U-L；本 task 只需複核已刪、勿重複處置）／
  **B-071 關帳**（隨 T002 之 PATCH）／B-020 改寫（per-IP 半邊關、通用化半邊續留）／B-008 改寫
  ...
  ★**併入：本刀新開條目的觸發器複驗**（2026-08-16 補；本 task 條文寫於開工前、未涵蓋它們）。
  本刀期間新開 **B-074~B-082 共九條**，收刀前 MUST 逐條複驗兩件事：
  ①**觸發條件是否仍成立**——本 repo 有過「四筆失效觸發器重定」的前例（條目掛在已經走過的刀上、
  永遠等不到觸發）。特別查那些寫「下一把動 X 的刀」而 X 已在本刀被動過的；
  ②**前提是否已被本刀後段推翻**——已知至少一例需判斷：**B-076**（`cache::publish` 回傳值零
  消費者）立條時 `reload_and_publish` **尚無生產面呼叫者**，而 U-H 之後它有了；條目結論
  （單副本下無害且刻意）是否仍成立、觸發器（拍板多副本）是否仍是對的等待點。
...
2. **鏈超長即拒絕，且在 app 層做**（★user 明示「一定要在 app 層做」）。判準取
   **跳數 > `MAX_XFF_TOKENS`**（＝現行取窗上界；不新增門檻、不新增要同步的常數）。
   副效果（**帶條件**）：**登入端點上**被服務的請求，其鏈皆 ≤32 跳 ⇒ **在窗未逾字元上限的
   前提下**，稽核欄裝的就是整條鏈。★該句**必帶兩個成立條件**、不得寫成全稱（兩者各有反例，且第二個已由憲法 v1.5.1 以逐位元反例證偽）：**(a) 限登入端點**——拒絕腿只掛在 login，其餘端點逾限照常服務（標記全域、拒絕僅登入，憲法 v1.6.2）；**(b) 窗未逾字元上限**——`XFF_MAX_CHARS`（1024）的字元兜底會切掉最左欄，而那一欄可能正是真實來源。
