...
- **FR-008**: addUser MUST 收帳號名（形制守門、現役唯一）、初始密碼（走政策、走冷卻、寫 custody）、暱稱、性別、
  ...
  空字串→NULL；密碼雜湊於鎖前計算。
  > ★**2026-08-30 裁定（收刀、user 親決；ADR 0069）**：手機的「**≤32**」係**欄位描述性上界、非 MUST 守門**。
  > 四權威三說法——契約 §3 守門序無此格、`msg-keys.md` 無手機鍵、DB `user_phone` 無長度上限（實測）、
  > rev4 藍本亦無守門，僅本條與 brainstorm G16 寫了該數字，而該數字在全 repo 找不到技術依據。
  > ⇒ 取「無守門」側，**碼零改動、零新拒因**；翻案觸發器（真實長度約束來源三種）逐字見 ADR 0069。
