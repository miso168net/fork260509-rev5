...
**Tests**: 本 repo TDD＝憲法級（§I.4）——各 story 之「驗證先行」任務**非可選**；驗證器
先紅、實作至綠。
...
- [ ] T002 [P] vendored 拷貝 `rust-api/sea-orm-adapter/`（整檔自 rev4 @ 2b8a101、§I.5
      例外）：僅 Cargo.toml 檔頭 provenance 註解行前移一代（拷貝自 rev4…原鏈 rev2@1fa2ebd
      →rev1@0b64a57）、其餘位元零改寫——驗收＝`diff -r` 對 rev4 源僅 provenance 行差異
- [ ] T003 [P] 立兩支 ADR draft：`docs/arc42/decisions/0005-schema-baseline-flatten-finalize.md`
      （基線＝rev4 終態壓平＋user 定稿制；provenance rev4:0014＋rev4:0021）＋
      `docs/arc42/decisions/0006-schema-gate-managed-evolution.md`（閘契約＝Day-1 受管
      演進帳；承 K1-32／K1-39）——status: draft
...
- [ ] T006 [US1] 結構驗證腳本（scratchpad `check-m001.py`、clarify extract.py 同構）：
      照相三查詢＋vs data-model §2 欄序比對＋rename 映射 vs rev4 快照全等比對；
      對空庫跑＝紅（15 表缺席逐項指名）——先紅為證
...
- [ ] T007 [US1] 實作 `rust-api/migration/src/m001_baseline_schema.rs`＋掛載 `src/lib.rs`：
      ...
      反向——驗收＝T006 綠＋up→down 後 public 淨空（除 seaql_migrations）→再 up 綠
...
- [ ] T008 [US2] seed 驗證＋轉錄雙腳本（scratchpad）：①`check-m002.py`——萃取實庫 vs
      ...
      ②`transcribe-m002.py`——自 seed-decision.json 機器轉錄 m002 用 SQL 字面（斷言列數
      266／PHC／時戳／protected true×17；★禁手抄、data-model §8）
...
- [ ] T011 [US3] 整組重建 `tools/schema-gate.py`（契約＝contracts/gates.md 逐條）：
      ...
      （結構／欄序／seed 值／sequence 落值各≥1、全紅）——驗收＝`test` 全綠＋fixtures
      缺席時 check rc 2 附補救
...
- [ ] T013 [US3] 演進帳往返驗證（quickstart C 四步實跑）：注入未登記欄→check rc 1 指名；
      補登記（E-001 試登）→rc 0；登記檔刪 date 欄→rc 2 啟動斷言；全還原（撤登記＋DROP、
...
- [ ] T015 [P] [US4] entity crate 15 檔：`rust-api/entity/Cargo.toml`（sea-orm features
...
- [ ] T017 [US4] entity-drift 恢復實跑驗證（依賴 T015＋T016）：`python3
...
- [ ] T018 [US4] `tools/docs-sync.py` 拔項＋真表首算（依賴 T010＋T016）：DAY1_EXEMPTIONS
      ...
      `lint` 全綠且跳過明細零 gen.snapshots——驗收＝pre-commit 全鏈綠（工具本體 staged
      觸發 docs-sync 自測）
...
- [ ] T019 [P] 活書隨刀：`docs/arc42/ARCHITECTURE.md` 資料慣例節補 memo 欄家族一行
      （data-model §5 語意；UI 兌現＝B-003）＋schema 基線現況一段（15 表／archetype／
      閘與演進帳、現在式）
- [ ] T020 [P] ADR 0005／0006 draft→accepted（body 不變、status 翻轉；DECISIONS-INDEX
      由 generate 重算）
...
- T005←T001；T007←T005+T006；T009←T007+T008；T011←T010（audit 左源）；T012←T009+T011；
  T013←T012；T015←T005（workspace）；T016←T009；T17←T015+T016；T018←T010+T016；
  T021←全部。

### Parallel Opportunities

- Setup：T002／T003／T004 三支並行（T001 後）。
- US3：T010、T014 與 T011 不同檔可並行。
