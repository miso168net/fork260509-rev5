...
`deleted_at IS NULL` filter——R3-6）。
...
| wire 欄 | 型 | 三態語意 |
|---|---|---|
| settingKey | string（必） | 定位鍵；未知鍵→`2222`（Clarify Q3） |
| settingValue | string（必） | 新值；經 registry 驗證＋正規化；**JSON null＝顯式清空 NOT NULL 欄→`2222`** |
| description | 三態（缺席／null／值） | 缺席＝不動；null＝清空落 NULL；值＝設值（不經 registry、僅長度界 varchar 承載） |
...
## §4 route 註冊表（ROUTES const；RouteDef 六欄承 rev4 形）
...
- **dev identity 查表**（`#[cfg(debug_assertions)]`、research R8）：
  ...
  無標頭／token 不在表→`8888`。release 建置＝驗證器缺席、一切請求 `8888`（fail-closed）。
