...
2. **base-web inline？**：PASS——恰三新檔（rev5-settings.d.ts／rev5-settings.ts／zh-tw.ts）、零既有檔改動。前二在 §III.1 ADAPT／WRAPPER 軌道；zh-tw.ts 屬治理契約既定錨點（啟動書 Day-1⑦跨端契約閘＋MSG_DICT_LOCALES 常量自創世錨定該路徑），★射程釋義已固化＝ADR 0021（user 拍板 2026-08-08）：純新增檔不觸 §III.2「base-web inline」授權射程、zh-tw.ts＝裸 object export 孤立檔（無 App.I18n.Schema 標註、不接 runtime）、依 §III fork-delta 新增型紀律檔頭標記；en-us.ts 零改動（ADR 0020 甲案）；typecheck 驗收＝T011／T031。
...
7. **§III ★軌道？**：PASS——零 inline＝不觸 ★軌道（授權射程釋義＝ADR 0021）；首個 ★軌道（view／i18n runtime 接線＋zh-tw.ts 標型重構）依 ADR 0018/0020/0021 延前端刀一次開齊。
