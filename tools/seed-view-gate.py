...
════════ 具名豁免與到期語意（B-088 拍板／FR-049） ════════
  ...
  `EXEMPT` 常數表恰一列＝B-008 餘一張尚未兌現的 view（seed 列 77 `view.manage_audit`）、
  附 B-008 指針。★**到期即紅**：被豁免鍵的 view 一旦存在（導出集
  命中）即紅、逼人摘掉該列——豁免只能縮、不能養；★**幽靈亦紅**：豁免鍵已不在 seed 集＝掛空擋、
  同樣逼摘。★「恰一列＋附 B-008 指針」與「表不得取自輸入（args／env／讀檔）」這兩條不變式是
  **機器強制**的、不是註解裡的一句話——self-test 案 I 四根釘死：I-a 斷言 `EXEMPT` 鍵集恰為那一鍵、
  且每個值含「B-008 餘一張 view」字面；I-b 斷言 `cmd_check` 內 `run_gate` 的第四實參逐字＝`EXEMPT`
  ...
  「常數表養第二列」（佈線本身沒動）、I-c／I-d 各封一條讀檔擴充路（前者在判定函式內、後者在呼叫端
...
# ★本表內容（恰一列、附 B-008 指針）由 self-test 案 I-a 機器釘死：養第二列＝self-test 當場紅，
#   要養就得連同改一行斷言、在 diff 裡現形（＝拍板級可見）——豁免只能縮、不能養不是勸告。
EXEMPT = {
    "view.manage_audit": "B-008 餘一張 view（seed 列 77 manage_audit；頁面兌現後摘列）",
}
...
def self_test():
    ...
    #   `grep -c EXEMPT` 曾為 0）。於是摘紅燈有四條零反應的繞道：①養第二列豁免蓋掉真缺頁
    ...
    # I-a 常數表釘死：鍵集恰一列（B-088 拍板／FR-049）、附 B-008 指針。
    assert set(EXEMPT) == {"view.manage_audit"}, (
        "self-test I-a：EXEMPT 鍵集非 B-088 拍板的恰一列——養豁免是拍板級（須連同改本行斷言），"
        f"實得 {sorted(EXEMPT)}")
...
def main(argv):
    ...
    if cmd == "test":
        ...
        print("[seed-view-gate] ✓ self-test 過（A 缺 view 紅＋指名／B 全在綠／C1 豁免生效綠／C2 豁免到期紅／"
              ...
              "G seed 塊內解析／"
              "H imports 鍵解析／I 豁免常數表恰一列＋呼叫點原封佈線＋取值形封鎖＋就地改表封鎖）")
        return 0
    if cmd != "check":
