...
════════ 具名豁免與到期語意（B-088 拍板／FR-049） ════════
  ...
  `EXEMPT` 常數表已**歸零**＝B-008 兩張 view（system-settings＋audit）全數兌現、最後一列
  `view.manage_audit` 於 audit 頁進場時摘除；表結構與判定機制保留，日後新開豁免＝拍板級
  （須連同改 self-test 斷言、diff 現形）。★**到期即紅**：被豁免鍵的 view 一旦存在（導出集
  命中）即紅、逼人摘掉該列——豁免只能縮、不能養；★**幽靈亦紅**：豁免鍵已不在 seed 集＝掛空擋、
  同樣逼摘。★「表歸零」與「表不得取自輸入（args／env／讀檔）」這兩條不變式是
  **機器強制**的、不是註解裡的一句話——self-test 案 I 四根釘死：I-a 斷言 `EXEMPT` 為空表（養第一列
  即紅）、其值字面迴圈（守「恢復的豁免必附指針」）於空表下零次迭代＝暫時 vacuous（L-063 於斷言處
  就地揭露、鑑別力隨豁免恢復而恢復）；I-b 斷言 `cmd_check` 內 `run_gate` 的第四實參逐字＝`EXEMPT`
  ...
  「常數表養第一列」（佈線本身沒動）、I-c／I-d 各封一條讀檔擴充路（前者在判定函式內、後者在呼叫端
...
# ★本表已**歸零**（B-008 兩張 view 全數兌現；audit 頁進場摘除最後一列 view.manage_audit）；
#   空表由 self-test 案 I-a 機器釘死：養第一列＝self-test 當場紅，要養就得連同改一行斷言、
#   在 diff 裡現形（＝拍板級可見）——豁免只能縮、不能養不是勸告。
EXEMPT = {}
...
def self_test():
    ...
    #   `grep -c EXEMPT` 曾為 0）。於是摘紅燈有四條零反應的繞道：①養第一列豁免蓋掉真缺頁
    ...
    # I-a 常數表釘死：表歸零（B-008 兩張 view 全數兌現後的空表；B-088 拍板／FR-049）——
    #     養第一列即紅、要養就得連同改本行斷言、在 diff 裡現形（＝拍板級可見）。
    assert EXEMPT == {}, (
        "self-test I-a：EXEMPT 非空表——B-008 已全數兌現、豁免只能縮不能養；"
        f"要新開豁免（拍板級）須連同改本行斷言，實得 {sorted(EXEMPT)}")
    # ★L-063 就地揭露：下方值字面迴圈於空表下**零次迭代＝暫時 vacuous**（恆真、無鑑別力）；
    #   刻意保留不刪——它守的是「日後若恢復豁免，值必須帶指針字面」，恢復第一列的當下（上方
    #   空表斷言連同改掉之時）即恢復鑑別力；指針字面屆時一併改為新豁免所屬帳號。
    for _k, _why in sorted(EXEMPT.items()):
        assert "B-008 餘一張 view" in _why, \
...
def main(argv):
    ...
    if cmd == "test":
        ...
        print("[seed-view-gate] ✓ self-test 過（A 缺 view 紅＋指名／B 全在綠／C1 豁免生效綠／C2 豁免到期紅／"
              ...
              "G seed 塊內解析／"
              "H imports 鍵解析／I 豁免常數表歸零＋呼叫點原封佈線＋取值形封鎖＋就地改表封鎖）")
        return 0
    if cmd != "check":
